		...
		if (ef != null )
		{
			...
			while (!Done)
			{
				...
				if ( Option.equals( "3" ) )
				{
					...
					System.out.println( "\n*** Enter the topic to send message to [ Temperature / Humidity / Heater / Chiller / Humidifier / Dehumidifier ] *** " );
					topic = UserInput.KeyboardReadString();

					// Here we send the message to the message manager.

			    	try
			    	{
						ef.SendMessage( Msg, topic );
				...
				if ( Option.equals( "4" ) )
				{
			    	...
			    	try
			    	{
						...
						for (int j = 0; eq != null && j < eq.size(); j++) {
							System.out.println(
									eq.get(j).GetSize() + " messages received...");
							System.out.println("=========================");
							...
							for (int i = 0; i < qlen; i++) {
								...
								System.out.print((i + 1) + "::Sender ID: " + Msg
										.GetSenderId());
								System.out.print(":: Message ID:: " + Msg
										.GetMessageId());

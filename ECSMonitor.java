...
class ECSMonitor extends Thread
{
	...
	public void run()
	{
		...
		Vector<MessageQueue> eq = null;			// all the Message Queue of all topics
			...
			*********************************************************************/
			...
			while ( !Done )
			{
				...
				for (int j = 0; j < eq.size(); j++) {
					int qlen = eq.get(j).GetSize();

					for (int i = 0; i < qlen; i++) {
						Msg = eq.get(j).GetMessage();

						if (Msg.GetMessageId() == 1) // Temperature reading
						{
							try {
								CurrentTemperature = Float.valueOf(Msg.GetMessage())
										.floatValue();
					...
					mw.WriteMessage("Temperature:: " + CurrentTemperature + "F  Humidity:: "
							+ CurrentHumidity);
					...
					if (CurrentTemperature < TempRangeLow) // temperature is below threshhold
					{
						...
						if (CurrentTemperature
						    > TempRangeHigh) // temperature is above threshhold
						{
							...
							ti.SetLampColorAndMessage("TEMP OK",
										  1); // temperature is within threshhold
							Heater(OFF);
							Chiller(OFF);
					...
					if (CurrentHumidity < HumiRangeLow) {
						hi.SetLampColorAndMessage("HUMI LOW",
									  3); // humidity is below threshhold
						Humidifier(ON);
						Dehumidifier(OFF);
						...
						if (CurrentHumidity > HumiRangeHigh) // humidity is above threshhold
						{
							...
							hi.SetLampColorAndMessage("HUMI OK",
										  1); // humidity is within threshhold
							Humidifier(OFF);
							Dehumidifier(OFF);
	...
	***************************************************************************/
	...
	private void Dehumidifier( boolean ON )
	{
		...
		try
		{
			em.SendMessage( msg, "Dehumidifier" );
